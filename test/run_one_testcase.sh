...
if ls ${DIRECTORY}/mips_cpu/*.v &>/dev/null; then
   ...
   iverilog -g 2012 \
      ...
      -Pmips_cpu_${VARIANT}_tb.INSTR_MEM_INIT_FILE=\"test/1-binary/instr_${TESTCASE}.hex.txt\" -Pmips_cpu_${VARIANT}_tb.ANSWER_FILE=\"test/4-reference/${TESTCASE}.txt\" -Pmips_cpu_${VARIANT}_tb.BRANCH_JUMP_INIT_FILE=\"test/5-memory/test_loadj.txt\" -o test/2-simulator/mips_cpu_${VARIANT}_tb_${TESTCASE}\
      -Pmips_cpu_${VARIANT}_tb.WAVES=\"test/6-waveforms/${TESTCASE}.vcd\"
else
